...
export const fetchPlants = async (): Promise<
  AxiosResponse<PlantDTO[]>
> /* TODO */ => {
...
export const submitProduct = (
  ...
  if (type === "create") {
    return `新增商品成功：${(product as CreateProduct).title}`; //斷言
  }
  return `更新商品成功：${(product as UpdateProduct).id}`; //斷言
};
